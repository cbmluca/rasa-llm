import { state, ENTITY_FIELD_CONFIG } from '../helpers/shared.js';
...
function getEntityOptions(tool) {
  ...
  return entries
    .filter((entry) => entry && (entry[config.field] || entry.id))
    .map((entry) => ({
      value: String(entry[config.field] || entry.id || ''),
      label: config.label(entry),
      entity: entry,
    }))
...
function getEntitiesMatchingTitle(tool, title) {
  ...
  return getEntityOptions(tool).filter((option) => {
    const candidate = (option.entity?.title || option.label || '').trim().toLowerCase();
