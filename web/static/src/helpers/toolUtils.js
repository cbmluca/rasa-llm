import { CALENDAR_FIELD_LAYOUT, FIELD_LAYOUTS, TOOL_ACTION_FIELD_CONFIG, TOOL_REQUIRED_FIELDS } from './shared.js';
...
// WHAT: determine whether a field is required for a tool/action.
// WHY: shared validation logic should live next to other field helpers.
// HOW: check action overrides before falling back to tool defaults and special-case actions.
export function isFieldRequired(tool, action, field) {
  if (!tool) return false;
  const normalizedAction = action || '';
  const overrideRequired = TOOL_ACTION_FIELD_CONFIG[tool]?.[normalizedAction]?.required;
  ...
  if (tool === 'todo_list' && normalizedAction === 'list') {
    return false;
  }
  if (tool === 'calendar_edit' && normalizedAction === 'list') {
    return false;
  }
  if (tool === 'kitchen_tips' && normalizedAction !== 'create') {
    return false;
  }
  if (tool === 'app_guide' && normalizedAction !== 'create') {
    return false;
  }
  return true;
}
