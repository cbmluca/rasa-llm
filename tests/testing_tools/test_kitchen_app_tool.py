"""Integration tests covering parser→tool flows for kitchen tips and app guide."""
...
def test_app_guide_get_update(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    storage_path = tmp_path / "app_guide.json"
    monkeypatch.setattr(app_guide, "_DEFAULT_STORAGE_PATH", storage_path)

    create_cmd = 'Add App Guide section "tier_policies" titled "Tier Policies" with content "Initial"'
    ...
    update_cmd = "Update the App Guide entry for tier_policies with a note about governance."
    ...
    get_cmd = 'Get App Guide section "tier_policies"'
