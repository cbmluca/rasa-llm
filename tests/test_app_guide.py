...
def test_upsert_section_persists_payload(tmp_path: Path) -> None:
    ...
    entry = store.upsert_section("overview", "Overview", "Initial content")

    assert entry["section_id"] == "overview"
...
def test_upsert_section_overwrites_existing_data(tmp_path: Path) -> None:
    store = AppGuideStore(tmp_path / "app_guide.json")
    first = store.upsert_section("overview", "Overview", "Initial content")
    second = store.upsert_section("overview", "Overview", "Updated content")

    assert second["section_id"] == "overview"
